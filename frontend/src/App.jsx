  ...
  return (
    <div className="h-screen bg-gradient-to-b from-[#0d0f14] to-[#111923] text-white flex flex-col">
      ...
      <header className="p-4 flex items-center justify-between border-b border-white/10 backdrop-blur-xl">
        <h1 className="text-xl font-bold tracking-wider text-[#6bffcb] drop-shadow-[0_0_6px_#6bffcb]">
          SONAR
        </h1>
      ...
      <div className="flex-1 overflow-y-auto relative">
        <div className="pb-24">
          {tab === "scan" && (
            <ScanTab result={result} setResult={setResult} history={history} setHistory={setHistory}/>
          )}
            ...
            [
              ...
              <button
                ...
                className={`flex flex-col text-xs ${
                  tab === b.id ? "text-[#6bffcb]" : "text-gray-400"
                }`}
              >
                <div className="w-6 h-6 mx-auto">{b.icon}</div>
                {b.id.toUpperCase()}
              </button>
            ))
          }
...
/* ✅ SCAN TAB (unchanged except header removed) */
      ...
      {status==="listening" && audioData.length>0 && (
        <div className="absolute">
          {audioData.slice(0,28).map((val,i)=>{
            ...
            return (
              <div key={i} className="absolute w-1 bg-[#6bffcb] rounded-full"
                style={{
                  height:6+(val/255)*18,
                  left:"50%", top:"50%",
